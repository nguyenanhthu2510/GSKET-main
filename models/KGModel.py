...
# không chỉ tính attention trên các node mà còn đa mối quan hệ giữa các edge vào như một bias vào attention
...
class KGModel_v1(nn.Module):
    ...
    KG_ENTITY_DIM = 400
    NER_BATCH_SIZE = 256
    MAX_KNOWS = 60  # 知识的最大数量 The maximum amount of knowledge
    ...
    def __init__(self, args, tokenizer):
        ...
        # 知识实体的embedding embedding of knowlwdge entities
        ...
        # 图像和医学知识的Attention     Image and medical knowledge of Atention
        self.know_att = MHA_FF(d_model=args.d_model,
                               d_ff=args.d_ff,
                               h=args.num_heads,
                               dropout=args.dropout)
 
        # 加载医生标注的知识图谱的节点和边，用于bias Loads the nodes and edges of the knowledge graph annotated by the doctor for bias
    ...
    def forward(self, data, mode='train', epoch=0):
        ...
        # 提取视觉信息 Extract visual information
        att_feats, fc_feats, label_feats = self.visual_encoder(images)

        bsz, _, d_model = att_feats.shape
        # 视觉特征在医生手工标注的的知识图谱上attention The visual features are on the knowledge graph that the doctor has manually annotated
        ...
        # 合并用于最后的生成 Merge is used for the final build
...
class KGModel_vWOExternalKG(KGModel_v1): # with out External KG
    def forward(self, data, mode='train', epoch=0):
        images = data['images']
        targets = data['targets']
        knowledges = data['knowledges']
...
class KGModel_vWOInternalKG(KGModel_v1): # with out Internal KG
    def forward(self, data, mode='train', epoch=0):
        images = data['images']
        targets = data['targets']
        knowledges = data['knowledges']
...
class KGModel_vWMHA(KGModel_v1):    # Comparative experiment: Replace GMHA with regular MHA
    """对比实验
    把GMHA换成普通的MHA
    """
    def __init__(self, args, tokenizer):
