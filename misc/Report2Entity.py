...
# Các bước tiền xử lý để tăng tốc độ đào tạo
# Trích xuất các thực thể từ báo cáo bằng Stanza
    ...
    def extract(self, dataset='iu'):
        ...
        for split in self.reports.keys():
            for item in tqdm(self.reports[split]):
                ...
                # 将同一类型的实体放在一起，避免语义混乱    Đặt các thực thể cùng loại với nhau để tránh nhầm lẫn về ngữ nghĩa
...
def read_facts(knowledge_file, cache=None):
    ...
    # 返回：{头实体： 字符串形式的知识}
    # Đọc kiến ​​thức từ cơ sở kiến ​​thức ba chiều của RadGraph
    # Trả về: {thực thể đầu: kiến ​​thức ở dạng chuỗi}
    if not cache:
        cache = '../data/cache'
...
def helper(knowledge_file, cache=None):
    ...
    # 返回：{头实体： 字符串形式的知识}
    # Đọc kiến ​​thức từ cơ sở kiến ​​thức ba chiều của RadGraph
    # Trả về: {head thực thể: kiến ​​thức ở dạng chuỗi}
    if not cache:
        cache = '../data/cache'
